...
def _enforce_region_scope(question: str, answer: str) -> str:
    ...
    # Drop bullets that are purely West rows.
    lines = out.splitlines()
    kept: list[str] = []
    for ln in lines:
        if re.search(r"^\s*[-*]\s*west\b", ln, flags=re.IGNORECASE):
            continue
        kept.append(ln)
    out = "\n".join(kept)
...
def _cache_key(question: str) -> str:
    ...
    return "v2|" + " ".join((question or "").strip().lower().split())
...
def _run_single_question(
    ...
    *,
    ...
    answer = _bold_standard_headings(answer)
