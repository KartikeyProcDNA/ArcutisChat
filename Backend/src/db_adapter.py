...
def _pg_fetch_live_base_table_names(*, max_names: int = 800) -> List[str]:
    from postgres_runner import fetch_live_base_table_names as _fn

    return _fn(max_names=max_names)


def _pg_get_table_columns_hint(table_name: str) -> str:
    from postgres_runner import get_table_columns_hint as _fn
...
def _pg_live_table_names_prompt_text(
    *,
    ...
    from postgres_runner import live_table_names_prompt_text as _fn
...
def _pg_run_query(
    ...
    from postgres_runner import run_query as _fn
